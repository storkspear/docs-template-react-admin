...
`nav.tsx`(`NAV_ITEMS`)에 라벨이 있는 **메뉴 화면 12개** + 라벨 없이 라우트만 등록된 상세 화면 3개(앱 상세 `/apps/:slug` · 게시물 상세 `/content/:id` · 전체 이벤트 `/analytics/events`)로 구성돼요. 각 화면이 무엇을 보여주는지, 어떤 API 를 쓰는지, 모바일에서 어떻게 달라지는지를 정리했어요.

사이드바 메뉴는 세 **그룹**으로 나뉘어요(`nav.tsx`의 `group`/`scope` — § "공통 패턴" 참고):

| 그룹 | 화면 | 노출 조건 |
|---|---|---|
| **공통** | 대시보드 · 매출 분석 · 역할·권한 · 설정 | 항상 (`scope: 'global'`) |
...
- **계정 관리**: 계정 목록(이메일·표시이름·역할 태그) + "관리자 추가" 모달(이메일·비밀번호·역할 — 자기보다 낮은 티어만) + 역할 변경 셀렉트 + 비밀번호 재설정 모달 + 삭제(확인 모달). 본인·마지막 master 는 삭제/강등이 서버에서 거부돼요.
...
계정 정보(이메일 표시), 테마(라이트/다크), 본인 비밀번호 변경, 표 밀도, 알림 토글을 보여주는 "폼 페이지" 표준 레퍼런스예요.

- **API**: `POST /me/password` — **비밀번호 변경 폼은 실제 동작**해요(`changeMyPassword`, 현재 비밀번호 불일치면 `ADMIN_016` 메시지 표시). 테마 모드도 `useThemeMode`로 실제 동작(localStorage 지속). 나머지(표시 이름·표 밀도·알림 토글)는 스켈레톤이라 저장해도 유지 안 됨.
...
- **액션**(`PERM_FILES_WRITE` — `canWrite('/files')` 게이팅):
  ...
  - **삭제**: **사유 입력 모달(필수)** → `DELETE .../files?key=` — 즉시 영구삭제가 아니라 **soft-delete**예요. "삭제 대상으로 옮겼어요 — 30일 후 영구삭제돼요." 토스트가 뜨고, "삭제 대상" 탭에 삭제일·사유·**영구삭제 D-day**(`purgeAt`) 컬럼과 함께 나타나요.
...
- **API**: `GET /apps/{slug}/payments?query=&channel=&status=&type=&from=&to=&page=&size=`, `POST /apps/{slug}/payments/{paymentId}/refund` (환불), `GET /apps/{slug}/payments/{paymentId}/refunds` (환불 이력)
...
- **환불 모달**(중앙): 결제금액/이미환불/환불가능 잔액 + 금액 입력([전액] 프리셋)·사유. 구독 결제는 **일할계산**(`금액 × 잔여일 ÷ 전체일`) 제안 버튼. 이미 환불된 건은 **환불 이력**(건별 금액·사유·시각) 리스트 표시. 잔액 다 환불 시 `REFUNDED`, 일부면 `PARTIALLY_REFUNDED`(재환불 가능). 환불 버튼은 `PERM_PAYMENTS_WRITE` 게이팅.
